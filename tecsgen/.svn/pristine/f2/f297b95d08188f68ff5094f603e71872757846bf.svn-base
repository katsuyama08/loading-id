...
#@caller_regions::Hash: { :entry_port => ["region1","region2"...] }
  ...
  def initialize( cell, option )
    ...
    parse_plugin_arg
    @@plugin_list << self
    if @cell.get_region.get_domain_type == nil then
      # ドメイン指定されていない
      cdl_error( "HRPSB9999 $1 is in region not specified domain", @cell.get_name )
    elsif @cell.get_region.get_domain_type.get_option != "kernel" then
      # カーネルドメイン指定されていない
...
EOT
      signature.get_function_head_array.each{ |func_head|
        ...
        file.print <<EOT
#{indent}  restrict_acptn_#{func_name} = #{aacpt};
EOT
      }
      file.print <<EOT
#{indent}};
