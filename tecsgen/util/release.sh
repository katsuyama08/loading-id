...
read_manifest(){
    ...
    manifest=$2
    LP=$3  # leading path
    echo "****" read_manifet $PACKAGE $manifest $LP
    if [ -f $manifest ]; then
...
if [ "$LIST_UNMANIFESTED_FILES" = "true" ];then
    ...
    ALL_FILES_LIST=`find . -name reference -prune -o -name .svn -prune -o -type f -print | sort`
    # echo "ALL_FILES_LIST"    ###########
    # echo $ALL_FILES_LIST    ###########
...
# すべてのファイルが存在するかチェック
    ...
    if [ -d "../$PACKAGE-$VERSION" ];then
         echo ../$PACKAGE-$VERSION already exist. delete it before doing release.sh -c
	 exit 1
    fi
...
# tar の実行
    ...
    tar cf - $ALL | ( cd ../$PACKAGE-$VERSION ; tar xf - )
    ( cd .. ; tar czf $PACKAGE-$VERSION.tgz $PACKAGE-$VERSION )
    if [ $? != 0 ]; then
