...
#   の責任を負わない．
#  
#  $Id: HRMPPlugin.rb 3089 2019-08-15 23:14:33Z okuma-top $
#++
...
#@class::String: クラス名
  ...
  def initialize( region, name, option )
    super
    dbgPrint "HRPPlugin: initialize: region=#{region.get_name}, className=#{name}, option=#{option}\n"
    @region = region
    @name   = name
    @class = option    # class name not checked
  end

  def add_through_plugin( join, current_region, next_region, through_type )
    # p "HRMPPlugin: add_through_plugin"
    ...
    # return []
    dbgPrint "HRMPPlugin: add_through_plugin: #{current_region.get_name}=>#{next_region.get_name}, #{join.get_owner.get_name}.#{join.get_definition.get_name}=>#{join.get_cell.get_name}.#{join.get_port_name}, #{through_type}\n"

    puts "===== HRMPPlugin: Join Check Start====="
    ...
    HRMPClassPlugin.add_inter_class_join_set join
    ...
    p "Join: " + (join.
                    ...
                    get_global_name).to_s +
      " current:" + current_class + 
      " next:" + next_class
    puts "**** #{current_class} to #{next_class}"
    ...
    if HRPKernelObjectManager.include_celltype?(join.
      ...
      # 別のクラスへの結合
      puts "**** RPC"
      # return [ :HRMPRPCPlugin, "noClientSemaphore=false,PPAllocatorSize=256" ]
      return [ :RPCPlugin, "noClientSemaphore=false,PPAllocatorSize=256" ]
    else
      # その他 (結合不可：エラー)
      puts "**** NG"
      return nil
  ...
  def joinable?(current_region, next_region, through_type )
    dbgPrint "HRMPPlugin: joinable? from #{current_region.get_name} to #{next_region.get_name} (#{through_type})\n"
    return true
  end
