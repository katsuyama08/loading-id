...
#   の責任を負わない．
#  
#  $Id: FMPHandlerPlugin.rb 3144 2020-05-04 01:30:26Z okuma-top $
#++
  ...
  def new_cell( cell )
    ...
    if root then
      ...
      if class_type == nil then
        ...
        # end
        if class_type == "OutOfClass" then
          cdl_error2( cell.get_locale, "FMH9999 $1: is placed OutOfClass region. Kernel objects (not handlers) must be placed in CLS_P1_xxx, CLS_P1ALL_xxx specified region (xxx:P1, P1ALL, FLAT. Processos number can vary).", cell.get_name )
        end
      end
    else
      raise "root is nil"
