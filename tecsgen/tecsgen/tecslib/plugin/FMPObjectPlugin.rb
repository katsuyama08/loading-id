...
#   の責任を負わない．
#  
#  $Id: FMPObjectPlugin.rb 3144 2020-05-04 01:30:26Z okuma-top $
#++
...
class FMPObjectPlugin < CelltypePlugin
  ...
  def new_cell( cell )
    ...
    if root then
      ...
      elsif class_type.get_option == "OutOfClass" then
        cdl_error2( cell.get_locale, "FMP9999 $1: is placed OutOfClass region. Kernel objects (not handlers) must be placed in CLS_P1_xxx, CLS_P1ALL_xxx specified region (xxx:P1, P1ALL, FLAT. Processos number can vary).", cell.get_name )
      end
    else
      raise "root is nil"
    end
  ...
  def gen_factory file
    ...
    @cell_list.each{ |cell|
      ...
      class_type = root.get_class_type
      option = class_type.get_option
      dbgPrint "FMPObjectPlugin#gen_factory: option=#{option} cell=#{cell.get_name}\n"
      if option != option_prev then
        if option_prev != nil then
          if option_prev != "OutOfClass" then
            f.print "}\n"
          end
        end
        if option != "OutOfClass" then
          f.print "CLASS(#{convert_class_name option}){\n"
          indent = "  "
        else
          indent = ""
        end
