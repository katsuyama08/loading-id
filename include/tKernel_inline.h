 ...
 *  の責任を負わない．
 * 
 *  $Id: tKernel_inline.h 285 2018-03-21 02:55:49Z ertl-hiro $
 */
 ...
 */
Inline ER
eKernel_multiRotateReadyQueue(ID domainId, PRI taskPriority)
{
	return(mrot_rdq(domainId, taskPriority));
}
 ...
 */
Inline ER
eKernel_getDomainId(ID *p_domainId)
{
	return(get_did(p_domainId));
}
 ...
 */
Inline ER
eKernel_multiGetLoad(ID domainId, PRI taskPriority, uint_t* p_load)
{
	return(mget_lod(domainId, taskPriority, p_load));
}
...
/*
 *  対象指定での指定した優先順位のタスクIDの参照
 */
Inline ER
eKernel_multiGetNthTask(ID domainId, PRI taskPriority, uint_t nth, ID* p_taskID)
{
	return(mget_nth(domainId, taskPriority, nth, p_taskID));
}
 ...
 */
Inline ER
eKernel_probeMemory(const void *base, size_t size,
								ID taskId, MODE probeMemoryMode)
{
	return(prb_mem(base, size, taskId, probeMemoryMode));
}
 ...
 */
Inline ER
eKernel_referMemory(const void *address, T_RMEM *pk_memoryStatus)
{
	return(ref_mem(address, pk_memoryStatus));
}
 ...
 */
Inline ER
eKernel_changeSystemOperationMode(ID somId)
{
	return(chg_som(somId));
}
 ...
 */
Inline ER
eKernel_getSystemOperationMode(ID *p_somId)
{
	return(get_som(p_somId));
}
...
/*
 *  対象指定でのタスクの優先順位の回転（非タスクコンテキスト用）
 */
Inline ER
eiKernel_multiRotateReadyQueue(ID domainId, PRI taskPriority)
{
	return(mrot_rdq(domainId, taskPriority));
}
...
/*
 *  実行状態のタスクが属する保護ドメインIDの参照（非タスクコンテキスト用）
 */
Inline ER
eiKernel_getDomainId(ID *p_domainId)
{
	return(get_did(p_domainId));
}
